...
def extract_ravdess_speaker_id_from_path(file_path: str) -> int:
    """
    Extract speaker id from RAVDESS actor folder:
    Radvess/Actor_01/... -> 1
    """
...
def format_ravdess_dataset_for_split(dataset: List[Dict]) -> List[Dict]:
    """
    Add required keys for each sample:
    {
      "path": filepath,
      "label": emotion_label,
      "speaker": speaker_id
    }
    Keeps existing keys for compatibility.
    """
    out: List[Dict] = []
...
def extract_wav2vec_features(signal: np.ndarray, sr: int) -> np.ndarray:
    """
    Extract raw wav2vec2 embeddings for one signal.
    Returns shape: (time_steps, hidden_size).
    """
    if sr != 16000:
...
def extract_wav2vec_features_batch(signals: List[np.ndarray]) -> List[np.ndarray]:
    """
    Extract raw wav2vec2 embeddings for a batch of signals.
    """
    if not signals:
        return []
...
def get_or_compute_features(
    ...
    """
    Get cached embedding for one file, or compute and cache.
    """
...
def prepare_features(
    ...
    """
    Build feature matrix X and label vector y using cache + pooling.
    """
    ensure_feature_dirs()
    _ = load_wav2vec2()
...
def prepare_mfcc_features(
    ...
    """
    Build MFCC feature matrix X and label vector y.
...
#smoke
    import random
    random.seed(42)
    ravdess = random.sample(ravdess, 50)
    cremad = random.sample(cremad, 50)
#smoke
    ...
    # WAV2VEC2 pooling study (cached embeddings reused; no redundant embedding compute)
    for pooling_method in pooling_methods:
        print(f"\n=== Running Pooling: {pooling_method} ===")

        # In-domain (RAVDESS -> RAVDESS)
        ...
        X_train_in, y_train_in_raw = filter_to_seen_labels(
            X_train_in, y_train_in_raw, seen_in
        )
        X_test_in, y_test_in_raw = filter_to_seen_labels(
            X_test_in, y_test_in_raw, seen_in
        )
        ...
        X_train_cross, y_train_cross_raw = filter_to_seen_labels(
            X_train_cross, y_train_cross_raw, seen_cross
        )
        X_test_cross, y_test_cross_raw = filter_to_seen_labels(
            X_test_cross, y_test_cross_raw, seen_cross
        )
        ...
        acc_cross, f1_cross, cm_cross = evaluate_model(
            model_cross, X_test_cross, y_test_cross
        )
        ...
        print(f"\n=== In-Domain (RAVDESS -> RAVDESS) [{pooling_method}] ===")
        print(f"Train samples: {len(y_train_in)}")
        print(f"Test samples: {len(y_test_in)}")
        print(f"Accuracy: {acc_in:.4f}")
        print(f"Macro F1: {f1_in:.4f}")
        print("Confusion Matrix:")
        print(cm_in)
        print("Label order:")
        print(list(label_encoder_in.classes_))

        print(f"\n=== Cross-Domain (RAVDESS -> CREMA-D) [{pooling_method}] ===")
        print(f"Train samples: {len(y_train_cross)}")
        print(f"Test samples: {len(y_test_cross)}")
        print(f"Accuracy: {acc_cross:.4f}")
        print(f"Macro F1: {f1_cross:.4f}")
        print("Confusion Matrix:")
        print(cm_cross)
        print("Label order:")
        print(list(label_encoder_cross.classes_))
    ...
    for pooling_method in pooling_methods:
        ...
        mfcc_results[pooling_method] = {
            ...
            "cross_domain": {
                "accuracy": float(acc_cross_mfcc),
                "f1": float(f1_cross_mfcc),
            },
        }

        print(f"\n=== In-Domain MFCC (RAVDESS -> RAVDESS) [{pooling_method}] ===")
        print(f"Accuracy: {acc_in_mfcc:.4f}")
        print(f"Macro F1: {f1_in_mfcc:.4f}")
        print("Confusion Matrix:")
        print(cm_in_mfcc)

        print(f"\n=== Cross-Domain MFCC (RAVDESS -> CREMA-D) [{pooling_method}] ===")
        print(f"Accuracy: {acc_cross_mfcc:.4f}")
        print(f"Macro F1: {f1_cross_mfcc:.4f}")
        print("Confusion Matrix:")
        print(cm_cross_mfcc)
    ...
    for pooling_method in pooling_methods:
        ...
        print(
            f"{pooling_method:<11} | {in_acc:.4f}        | {in_f1:.4f}       | {cr_acc:.4f}           | {cr_f1:.4f}"
        )
    ...
    best_cross_w2v = max(
        pooling_methods, key=lambda m: wav2vec2_results[m]["cross_domain"]["f1"]
    )
    best_cross_mfcc = max(
        pooling_methods, key=lambda m: mfcc_results[m]["cross_domain"]["f1"]
    )

    best_in_model = (
        "wav2vec2"
        if wav2vec2_results[best_in_w2v]["in_domain"]["f1"]
        >= mfcc_results[best_in_mfcc]["in_domain"]["f1"]
        else "MFCC"
    )
    best_cross_model = (
        "wav2vec2"
        if wav2vec2_results[best_cross_w2v]["cross_domain"]["f1"]
        >= mfcc_results[best_cross_mfcc]["cross_domain"]["f1"]
        else "MFCC"
    )
